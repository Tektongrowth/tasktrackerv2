...
export function EmbedLocationPage() {
  ...
  // Fetch client data by location ID
  const { data, isLoading, error } = useQuery<EmbedData>({
    queryKey: ['embed-location', locationId],
    queryFn: async () => {
      const res = await fetch(`/api/embed/location/${locationId}`);
      if (!res.ok) {
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/embed/location/${locationId}/submit`, {
        method: 'POST',
